...
from app.repositories.document_repository import InMemoryDocumentRepository, StoredDocument, StoredVersion
...
class DocumentService:
    ...
    def create_draft(self, command: CreateDocumentCommand) -> DocumentResponse:
        ...
        document = StoredDocument(
            uuid4(), self.repository.next_document_no(), command.applicant_id, command.applicant_department,
            command.total_amount, command.currency, command.apply_date, command.reason_text,
        )
        self.repository.documents[document.document_id] = document
        return self._response(document)

    def submit(self, document_id: UUID, actor_id: UUID, expected_state_version: int) -> DocumentVersionResponse:
        """提交草稿并创建不可变版本，检查乐观锁。"""
        ...
        return DocumentVersionResponse(version_id=version.version_id, document_id=version.document_id, version_no=version.version_no, created_by=version.created_by)

    def resubmit_after_return(self, document_id: UUID, actor_id: UUID, expected_state_version: int) -> DocumentVersionResponse:
    ...
    def _response(document: StoredDocument) -> DocumentResponse:
        """转换为 API 响应模型。"""
        return DocumentResponse(document_id=document.document_id, document_no=document.document_no, applicant_id=document.applicant_id, total_amount=document.total_amount, currency=document.currency, document_status=document.status, current_version=document.current_version, state_version=document.state_version)
